...
Version: 2.5 - Added Guild Manager & All APIs
"""
...
def join_clan_jwt(clan_id, jwt_token):
    return send_request(f"{GUILD_BASE_URL}/request_clan", {"clan_id": clan_id, "jwt": jwt_token}, "GET")

def join_clan_guest(clan_id, uid, password):
    return send_request(f"{GUILD_BASE_URL}/request_clan", {"clan_id": clan_id, "uid": uid, "pass": password}, "GET")

def join_clan_access(clan_id, access_token):
    return send_request(f"{GUILD_BASE_URL}/request_clan_access", {"clan_id": clan_id, "access_token": access_token}, "GET")

def leave_clan_jwt(clan_id, jwt_token):
    return send_request(f"{GUILD_BASE_URL}/quit_clan", {"clan_id": clan_id, "jwt": jwt_token}, "GET")

def leave_clan_guest(clan_id, uid, password):
    return send_request(f"{GUILD_BASE_URL}/quit_clan", {"clan_id": clan_id, "uid": uid, "pass": password}, "GET")

def leave_clan_access(clan_id, access_token):
    return send_request(f"{GUILD_BASE_URL}/quit_clan_access", {"clan_id": clan_id, "access_token": access_token}, "GET")
...
<head>
    ...
    <style>
        * {
        ...
        .method-toggle, .action-toggle {
            display: flex;
            gap: 1rem;
            margin-bottom: 1.5rem;
            justify-content: center;
        }
        .method-btn, .action-btn {
            flex: 1;
            background: rgba(0,0,0,0.5);
            border: 1px solid rgba(255,255,255,0.2);
            padding: 0.8rem;
        ...
        .method-btn.active, .action-btn.active {
            background: linear-gradient(95deg, #FF3CAC, #784BA0);
            color: white;
            border-color: transparent;
            box-shadow: 0 0 12px rgba(255,60,172,0.5);
        }
        .method-btn i, .action-btn i {
            margin-right: 8px;
        }
        button.submit-btn {
            width: 100%;
...
<div class="container">
    <div class="hero">
        ...
        <div class="tag"><i class="fas fa-bolt"></i> ITEM MANAGER & LONG BIO & GUILD</div>
    </div>
    ...
    <div class="top-menu">
        ...
        <button class="menu-btn" data-feature="guild"><i class="fas fa-users"></i> GUILD MANAGER</button>
        <button class="menu-btn" data-feature="allapis"><i class="fas fa-code-branch"></i> ALL APIS</button>
    </div>
...
<script>
    ...
    function attachFormHandlers(feature) {
        if (feature === 'guest') {
        ...
        } else if (feature === 'guild') {
            // Guild manager form
            const actionJoin = document.getElementById('actionJoin');
            const actionLeave = document.getElementById('actionLeave');
            ...
            const submitBtn = document.getElementById('submitGuild');
            ...
            function updateAction(action) {
                ...
                if (action === 'join') {
                    actionJoin.classList.add('active');
                    actionLeave.classList.remove('active');
                } else {
                    actionLeave.classList.add('active');
                    actionJoin.classList.remove('active');
                }
            }
            ...
            if (actionJoin && actionLeave) {
                actionJoin.onclick = () => updateAction('join');
                actionLeave.onclick = () => updateAction('leave');
            }
            ...
            if (submitBtn) {
                submitBtn.onclick = () => {
                    ...
                    if (!clanId) return showResult('guildResult', 'Clan ID is required', true);
                    let endpoint = '';
                    let payload = { clan_id: clanId };
                    if (currentAction === 'join') {
                        if (currentMethod === 'jwt') {
                            const jwt = document.getElementById('guildJwt').value.trim();
                            if (!jwt) return showResult('guildResult', 'JWT token required', true);
                            endpoint = '/api/join_clan_jwt';
                            payload.jwt = jwt;
                        } else if (currentMethod === 'access') {
                            const access = document.getElementById('guildAccess').value.trim();
                            if (!access) return showResult('guildResult', 'Access token required', true);
                            endpoint = '/api/join_clan_access';
                        ...
                        if (currentMethod === 'jwt') {
                            const jwt = document.getElementById('guildJwt').value.trim();
                            if (!jwt) return showResult('guildResult', 'JWT token required', true);
                            endpoint = '/api/leave_clan_jwt';
                            payload.jwt = jwt;
                        } else if (currentMethod === 'access') {
                            const access = document.getElementById('guildAccess').value.trim();
                            if (!access) return showResult('guildResult', 'Access token required', true);
                            endpoint = '/api/leave_clan_access';
                            payload.access_token = access;
                        } else {
                            const uid = document.getElementById('guildUid').value.trim();
                            const pwd = document.getElementById('guildPwd').value;
                            if (!uid || !pwd) return showResult('guildResult', 'UID and Password required', true);
                            endpoint = '/api/leave_clan_guest';
                            payload.uid = uid;
                            payload.password = pwd;
                        }
                    }
                    callApi(endpoint, payload, 'guildResult', 'Processing guild action...');
                };
            }
        } else if (feature === 'allapis') {
            // No actions needed for All APIs, just display.
        }
    }
    ...
    forms = {
        ...
        'guild': {
            ...
            'html': '''
                <div class="input-group">
                    <label>Clan ID</label>
                    <input type="text" id="clanId" placeholder="Enter clan ID">
                </div>
                ...
                <div id="guildJwtFields">
                    <div class="input-group">
                        <label>JWT Token</label>
                        <input type="text" id="guildJwt" placeholder="JWT token">
                    </div>
                </div>
                <div id="guildAccessFields" style="display:none;">
                    <div class="input-group">
                        <label>Access Token</label>
                        <input type="text" id="guildAccess" placeholder="Access token">
                    </div>
                </div>
                ...
                <button class="submit-btn" id="submitGuild"><i class="fas fa-users"></i> EXECUTE ACTION</button>
                <div class="result-area" id="guildResult">🏰 ready</div>
            '''
        },
        'allapis': {
            ...
            'html': '''
                <div class="api-list">
                    <div class="api-item"><div class="api-name">🔹 Guest Item Add</div><div class="api-url">https://rizerxprofileitemadder.vercel.app/add-profile-guest</div></div>
                    <div class="api-item"><div class="api-name">🔹 JWT Item Add</div><div class="api-url">https://rizerxprofileitemadder.vercel.app/add-profile</div></div>
                    <div class="api-item"><div class="api-name">🔹 Access Token Item Add</div><div class="api-url">https://rizerxprofileitemadder.vercel.app/add-profile-access-v2</div></div>
                    <div class="api-item"><div class="api-name">🔹 Access → JWT</div><div class="api-url">https://rizerxaccessjwt.vercel.app/rizer</div></div>
                    <div class="api-item"><div class="api-name">🔹 Guest → Tokens (JWT+Access)</div><div class="api-url">https://rizerxguestaccountacceee.vercel.app//rizer</div></div>
                    <div class="api-item"><div class="api-name">🔹 Long Bio (Guest)</div><div class="api-url">https://cleint-bio-changer.vercel.app/bio_upload</div></div>
                    <div class="api-item"><div class="api-name">🔹 Long Bio (JWT)</div><div class="api-url">https://cleint-bio-changer.vercel.app/bio_upload</div></div>
                    <div class="api-item"><div class="api-name">🔹 Guild Manager - Join/Leave (JWT/Guest)</div><div class="api-url">https://rizerxguildmanager.vercel.app/request_clan, /quit_clan</div></div>
                    <div class="api-item"><div class="api-name">🔹 Guild Manager - Join/Leave (Access)</div><div class="api-url">https://rizerxguildmanager.vercel.app/request_clan_access, /quit_clan_access</div></div>
                </div>
                <p style="margin-top:1rem; font-size:0.8rem; text-align:center;">📡 All external endpoints used by this dashboard</p>
            '''
        }
    }
...
# Guild Manager API endpoints
@app.route('/api/join_clan_jwt', methods=['POST'])
def api_join_clan_jwt():
    data = request.get_json()
    clan_id = data.get('clan_id', '').strip()
    jwt = data.get('jwt', '').strip()
    if not clan_id or not jwt:
        return jsonify({'error': 'Clan ID and JWT token required'}), 400
    result = join_clan_jwt(clan_id, jwt)
    return jsonify({'result': result})

@app.route('/api/join_clan_guest', methods=['POST'])
def api_join_clan_guest():
    data = request.get_json()
    clan_id = data.get('clan_id', '').strip()
    uid = data.get('uid', '').strip()
    password = data.get('password', '').strip()
    if not clan_id or not uid or not password:
        return jsonify({'error': 'Clan ID, UID and password required'}), 400
    result = join_clan_guest(clan_id, uid, password)
    return jsonify({'result': result})

@app.route('/api/join_clan_access', methods=['POST'])
def api_join_clan_access():
    data = request.get_json()
    clan_id = data.get('clan_id', '').strip()
    access_token = data.get('access_token', '').strip()
    if not clan_id or not access_token:
        return jsonify({'error': 'Clan ID and access token required'}), 400
    result = join_clan_access(clan_id, access_token)
    return jsonify({'result': result})

@app.route('/api/leave_clan_jwt', methods=['POST'])
def api_leave_clan_jwt():
    data = request.get_json()
    clan_id = data.get('clan_id', '').strip()
    jwt = data.get('jwt', '').strip()
    if not clan_id or not jwt:
        return jsonify({'error': 'Clan ID and JWT token required'}), 400
    result = leave_clan_jwt(clan_id, jwt)
    return jsonify({'result': result})

@app.route('/api/leave_clan_guest', methods=['POST'])
def api_leave_clan_guest():
    data = request.get_json()
    clan_id = data.get('clan_id', '').strip()
    uid = data.get('uid', '').strip()
    password = data.get('password', '').strip()
    if not clan_id or not uid or not password:
        return jsonify({'error': 'Clan ID, UID and password required'}), 400
    result = leave_clan_guest(clan_id, uid, password)
    return jsonify({'result': result})

@app.route('/api/leave_clan_access', methods=['POST'])
def api_leave_clan_access():
    data = request.get_json()
    clan_id = data.get('clan_id', '').strip()
    access_token = data.get('access_token', '').strip()
    if not clan_id or not access_token:
        return jsonify({'error': 'Clan ID and access token required'}), 400
    result = leave_clan_access(clan_id, access_token)
    return jsonify({'result': result})
